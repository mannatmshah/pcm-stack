...
$Transport_array=array("Value"=>"Key"); 
...
if($rs['firm_type']=='Transport'){
  //$transRow[$rowcount][0]=$rs['company_id'];
  //$transRow[$rowcount][1]=$rs['firm_name'];
  $trans_array=array($rs['firm_name']=>$rs['company_id']);
  $Transport_array=array_merge($Transport_array,$trans_array);  
...
while($rs=mysqli_fetch_array($result))
{
  ...
  */
  ...
  //$disp_transport_name=array_search($rs['transport_name'],$Transport_array);
