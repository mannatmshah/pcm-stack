...
SELECT bill_supplier_name, 
       ...
       (total_bill_amount -total_discount-total_gr -total_payment) AS total_os,
       pay_supp_company_id, 
       supp_comm,agent_id,
       city 
FROM (  SELECT supp_firm_name AS bill_supplier_name , 
    ...
    LEFT JOIN (SELECT SUM(PAY_BILL.dis_amount) AS total_discount, 
                ...
                FROM txt_payment_entry_main AS MAIN ,
                     txt_payment_bill_entry AS PAY_BILL 
                WHERE MAIN.delete_tag='FALSE' 
